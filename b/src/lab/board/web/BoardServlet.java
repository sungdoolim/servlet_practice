...
public class BoardServlet extends HttpServlet {
   ...
   public void init(ServletConfig config) throws ServletException{
	...
	String url = "/index.jsp?url=";		//get방식으로 넘어감
	if("write".equals(action)) {
		request.setAttribute("message", "새글 입력");
		url=url + "/board/write.jsp";
		request.setAttribute("action", "write_do");
	}
	
	else if ("detail".equals(action)) {
		request.setAttribute("message", "상세 내용 출력");
		url=url + "/board/detail.jsp";
		...
		int totalPage =0; //전체 페이지 출력  : 15개 게시물이면 페이지는 2개
		if(bbsCount>0) {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		else if("detail_do".equals(action)) {
			
			String password =request.getParameter("empass");
			String name =request.getParameter("employee");
			
			BoardVO board = new BoardVO();
			board.setName(name);
			board.setPassword(password);	
			String dbpw=dao.getPassword2(name);
			if(dbpw.equals(password)) {		
				
				BoardVO board2= new BoardDAO().selectArticle2(name);
				request.setAttribute("board", board2);
				
				url=url+"/board/detail2.jsp";
				
			}else {
				request.setAttribute("message", "비밀번호가 다릅니다. 내용이 출력되지 않습니다.");
				url=url+"/error/error.jsp";
			}
		}
